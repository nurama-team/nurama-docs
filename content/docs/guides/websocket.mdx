...
The WebSocket server is a separate Socket.IO deployment. If a server you run needs to react to changes without holding a connection open, use [webhooks](/docs/guides/webhooks) instead. Connect with a Socket.IO client (the SDK's `socket` namespace, or `socket.io-client` directly), never with a raw WebSocket.
